...
## Product Detail Page
- `client/src/pages/ProductDetail.tsx` is the awward-style redesign. Beden/renk seçiciler aktif: sync motoru artık `productMainId` bazında gruplama yapıyor (her model tek ürün, bedenleri `product_variants` tablosunda), dolayısıyla seçiciler gerçek varyantlar üzerinde çalışır. `handleAddToCart` passes `quantity` and the matched variant id; `addToCart(productId, variantId?, quantity?)` from `useCart`.
